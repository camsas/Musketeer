...
#ifndef MUSKETEER_SORT_OPERATOR_H
#define MUSKETEER_SORT_OPERATOR_H
...
class SortOperator : public OperatorInterface {
 public:
  SortOperator(const string& input_dir, const pANTLR3_BASE_TREE condition_tree,
               const vector<Relation*>& relations,
               Column* column_, bool increasing_, Relation* output_rel):
    OperatorInterface(input_dir, relations, output_rel,
                      new ConditionTree(condition_tree)),
      column(column_), increasing(increasing_) {
  }

  SortOperator(const string& input_dir,
  ...
  SortOperator(const string& input_dir, ConditionTree* condition_tree,
               const vector<Relation*>& relations,
               Column* column_, bool increasing_, Relation* output_rel):
