...
#ifndef MUSKETEER_MAX_OPERATOR_H
#define MUSKETEER_MAX_OPERATOR_H
...
class MaxOperator : public OperatorInterface {
 public:
  MaxOperator(const string& input_dir, const pANTLR3_BASE_TREE condition_tree,
  ...
  MaxOperator(const string& input_dir,
  ...
  MaxOperator(const string& input_dir, ConditionTree* condition_tree,
              const vector<Column*>& group_bys_,
              const vector<Relation*>& relations,
              const vector<Column*>& selected_columns_,
              Column* column_, Relation* output_rel):
