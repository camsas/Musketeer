...
namespace framework {
  ...
  using musketeer::translator::TranslatorGraphChi;
  using musketeer::ir::CountOperator;
  using musketeer::ir::JoinOperator;

  GraphChiFramework::GraphChiFramework(): FrameworkInterface() {
    dispatcher_ = new GraphChiDispatcher();
    monitor_ = NULL;
    //TODO(malte): add GraphChi Monitor
  }

  string GraphChiFramework::Translate(const op_nodes& dag,
                                      const string& relation) {
    TranslatorGraphChi translator_graphchi =
      translator::TranslatorGraphChi(dag, relation);
    return translator_graphchi.GenerateCode();
  }

  void GraphChiFramework::Dispatch(const string& binary_file,
                                   const string& relation) {
    if (!FLAGS_dry_run) {
  ...
  FmwType GraphChiFramework::GetType() {
    return FMW_GRAPH_CHI;
  }

  uint32_t GraphChiFramework::ScoreDAG(const node_list& nodes,
                                       const relation_size& rel_size) {
    node_set to_schedule;
    int32_t num_ops_to_schedule = 0;
    op_nodes input_nodes;
    // At the moment we only support one input operator DAGs in GraphChi.
  ...
  double GraphChiFramework::ScoreOperator(shared_ptr<OperatorNode> op_node,
                                          const relation_size& rel_size) {
    return 0.0;
  }

  // The framework runs on a single machine => cluster state of 0.
  double GraphChiFramework::ScoreClusterState() {
    return 0.0;
  }

  // Compile time for a GraphChi job is ~1s.
  double GraphChiFramework::ScoreCompile() {
    return 1.0 * FLAGS_time_to_cost;
  }

  double GraphChiFramework::ScorePull(uint64_t data_size_kb) {
    // This represents the expected duration of the phase in seconds.
    return data_size_kb / 50.0 / 1024.0 * FLAGS_time_to_cost;
  }

  double GraphChiFramework::ScoreLoad(uint64_t data_size_kb) {
    // This represents the expected duration of the phase in seconds.
    return data_size_kb / 12.0 / 1024.0 * FLAGS_time_to_cost;
  }

  double GraphChiFramework::ScoreRuntime(uint64_t data_size_kb,
    ...
    //   connected ones.
    //         2) consider the amount of operators executed in each vertex.
    return data_size_kb / 8.5 / 1024.0 * FLAGS_time_to_cost;
  }

  double GraphChiFramework::ScorePush(uint64_t data_size_kb) {
    // This represents the expected duration of the phase in seconds.
    return data_size_kb / 25.0 / 1024.0 * FLAGS_time_to_cost;
  }

  pair<bool, shared_ptr<OperatorNode> > GraphChiFramework::CanMergePreWhile(
  ...
  pair<bool, shared_ptr<OperatorNode> > GraphChiFramework::CanMergePreGroup(
      shared_ptr<OperatorNode> cur_node, set<string>* visited,
      const node_set& to_schedule, int32_t* num_ops_to_schedule) {
  ...
  pair<bool, shared_ptr<OperatorNode> > GraphChiFramework::CanMergePostGroup(
      shared_ptr<OperatorNode> cur_node, set<string>* visited,
      const node_set& to_schedule, int32_t* num_ops_to_schedule) {
    op_nodes children = cur_node->get_children();
    for (; children.size() > 0;
  ...
  bool GraphChiFramework::CanMerge(const op_nodes& dag,
