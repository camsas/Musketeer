...
###########################################################################
# Automatic C++ source base linting script,
# with pretty-print output.
#
# Copyright (c) 2013-14 Malte Schwarzkopf <malte.schwarzkopf@cl.cam.ac.uk>
###########################################################################
  ...
  for root, subfolders, files in os.walk(source_dir):
    for filename in files:
      if not (filename.endswith(".pb.cc") or filename.endswith(".pb.h") or \
          ...
          and not "wildcherry" in filename and not "RLPlus" in filename \
          and not "libchaste" in root \
          and filename.endswith(".h") or filename.endswith(".cc"):
        f = os.path.join(root, filename)
        # XXX(malte): Hack to ignore simulator for linting purposes (for now)
        if "/examples/" in f or "_templates/" in f:
          continue
        if verbose:
...
#                              stdout=sys.stdout, stderr=sys.stderr)
    ...
    try:
      retdata = subprocess.check_output(
          ["python", "ext/cpplint.py", "--linelength=100",
           "--filter=%s" % (filter_string), source_file],
